...
→ 停止 dsh → 原子替换 dsh-runtime → 重启 → 失败自动回滚上一版
```
...
- **单元测试**（vitest）：`runtime-resolver` 的择一与校验分支、`port-allocator` 的顺延逻辑、`settings-store` 的校验与默认值、`npm-updater` 的版本比较与回滚分支。
- **集成测试**：以一个假 Node 服务脚本冒充 dsh，驱动 `dsh-supervisor` 走完「启动 → 就绪 → 崩溃重启 → 退避上限 → 优雅关停」完整状态机。
- **手工验收清单**：全新安装；断网启动；端口被占用；更新成功；更新失败回滚；托盘与关窗行为；设置项改动生效。

## 9. 待实测确认项

- `@deepseek-ai/dsh` 生产依赖安装体积（决定最终安装包大小，预计 200–400MB）。
- 内置 Node 24 的最小可用文件集（能否只带 `node.exe`）。
